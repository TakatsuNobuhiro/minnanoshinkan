class Event < ApplicationRecord
  ...
  def create_notification_like!(current_user)
    temp = Notification.where(["student_visitor_id = ? and club_visited_id = ? and event_id = ? and action = ? ",
                                  current_user.id, club_id, id, 'event_like'])
    if temp.blank?
  ...
  def create_notification_comment!(current_user, comment_id)
    ...
    favorite_ids = EventFavorite.where(event_id: id).where.not("student_id=?", current_user.id).select(:student_id)
    temp_ids=[]
  ...
  def create_notification_event!(current_user)
    ...
    temp_ids=[]
    favorite_ids.each do |f|
      temp_ids.push(f["student_id"])
    end
    
    temp_ids.each do |temp_id|
      notification = current_user.active_notifications.new(
        event_id: id,
        student_visited_id: temp_id,
        action: 'event'
      )
      notification.save if notification.valid?
    end
