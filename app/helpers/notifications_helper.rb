module NotificationsHelper
  def notification_form(notification)
    ...
    when 'club_like'
      tag.a(notification.student_visitor.name, href: student_path(@student_visitor)) + 'があなたをいいねしました'      
    when 'comment' then
      #コメントの内容と投稿のタイトルを取得　      
    ...
    when 'event' then
      ...
      tag.a(@club.name, href: club_path(@club.id))+"が"+tag.a("新規イベント",href: event_path(notification.event_id))+"を投稿しました。"
    end
  end
